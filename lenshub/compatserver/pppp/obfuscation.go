package pppp

// The legacy CS2/PPPP obfuscator uses a 256-byte shuffle table. The table is
// protocol material documented by the MIT-licensed magicus/pppp-dissector.
// See lenshub/THIRD_PARTY_NOTICES.md in the parent repository.
var shuffleTable = [256]byte{
	,
}

func pskHash(psk string) [4]byte {
	var h [4]byte
	for i := 0; i < len(psk); i++ {
		b := psk[i]
		h[0] += b
		h[1] -= b
		h[2] += b / 3
		h[3] ^= b
	}
	return h
}

func lookupObfuscationByte(h [4]byte, previous byte) byte {
	idx := byte(h[previous&0x03] + previous)
	return shuffleTable[idx]
}

func Obfuscate(psk string, clear []byte) []byte {
	out := make([]byte, len(clear))
	h := pskHash(psk)
	var prev byte
	for i, b := range clear {
		out[i] = b ^ lookupObfuscationByte(h, prev)
		prev = out[i]
	}
	return out
}

func Deobfuscate(psk string, encrypted []byte) []byte {
	out := make([]byte, len(encrypted))
	h := pskHash(psk)
	var prev byte
	for i, b := range encrypted {
		out[i] = b ^ lookupObfuscationByte(h, prev)
		prev = b
	}
	return out
}
